...
float accx_nog=0,accy_nog=0,accz_nog=0;
...
static void Cmd_RxUnpack(U8 *buf, U8 DLen)
{
    ...
    case 0x11: // 获取订阅的功能数据 回复或主动上报
        ...
        {// 加速度xyz 去掉了重力 使用时需*scaleAccel m/s
            accx_nog = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2; Dbp("\taX: %.3f\r\n", accx_nog); // x加速度aX
            accy_nog = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2; Dbp("\taY: %.3f\r\n", accy_nog); // y加速度aY
            accz_nog = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAccel; L += 2; Dbp("\taZ: %.3f\r\n", accz_nog); // z加速度aZ
            tmpAbs = sqrt(pow2(accx_nog) + pow2(accy_nog) + pow2(accz_nog)); Dbp("\ta_abs: %.3f\r\n", tmpAbs); // 3轴合成的绝对值
        }
        if ((ctl & 0x0002) != 0)
        ...
        {// 角速度xyz 使用时需*scaleAngleSpeed °/s
            ...
            gyrox = tmpX;
            gyroy = tmpY;
            gyroz = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAngleSpeed; L += 2;
            tmpZ = gyroz;
            Dbp("\tGZ: %.3f\r\n", tmpZ); // z角速度GZ
        ...
        {// 四元素 wxyz 使用时需*scaleQuat
            quatw = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2; Dbp("\tw: %.3f\r\n", tmpAbs); // w
            quatx =   (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2; Dbp("\tx: %.3f\r\n", tmpX); // x
            quaty =   (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2; Dbp("\ty: %.3f\r\n", tmpY); // y
            quatz =   (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleQuat; L += 2; Dbp("\tz: %.3f\r\n", tmpZ); // z
        }
        if ((ctl & 0x0040) != 0)
        {// 欧拉角xyz 使用时需*scaleAngle
            Pitch_Angle = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAngle; L += 2; Dbp("\tangleX: %.3f\r\n", Pitch_Angle); // x角度
            Roll_Angle  = (S16)(((S16)buf[L+1]<<8) | buf[L]) * scaleAngle; L += 2; Dbp("\tangleY: %.3f\r\n", Roll_Angle);  // y角度
...
void IM948_Init(void)
{
     ...
     * @param Cmd_ReportTag 功能订阅标识
     */
    Cmd_12(5, 255, 0,  0, 3, 220, 1, 2, 9, 0x44);// 7 设置设备参数(只读取欧拉角)//0x40
