...
/*
 * 说明（重要）：
 * 本文件由 Simulink Coder 自动生成，用于将外部输入（rtU）转换为外部输出（rtY）。
 * 在本工程的用法里：
 * - rtU.W1 / rtU.W2：两路编码器/轮子增量（单位通常是“计数 counts”或已换算的位移增量）
 * - rtU.DEG：航向角（单位：度），内部会转换为弧度
 * - rtY.XOUT / rtY.YOUT：本周期计算得到的平面位移增量（Δx、Δy），供上层累加成位置
 *
 * 备注：生成代码中包含 Moving Average、Discrete-Time Integrator、Variable Integer Delay 等模块，
 * 用于对原始增量进行平滑/滤波（因此代码看起来比“直接公式”更复杂）。
 */
...
/* rtU = Real-Time (model) Inputs：上层在调用 IM_TEST_step() 前写入 */
ExtU rtU;

/* External outputs (root outports fed by signals with default storage) */
/* rtY = Real-Time (model) Outputs：IM_TEST_step() 计算后由上层读取 */
ExtY rtY;
...
void IM_TEST_step(void)
{
   ...
   *  Inport: '<Root>/In1'
   */
  /* rtU.DEG 为“度”，0.0174532924 = pi/180，将角度转为弧度 */
  ...
  /*
   * 关键计算（坐标分解/投影）：
   * 下面两行本质是在用 yaw（航向）把两路增量 W1/W2 投影到平面坐标系，得到两个正交分量。
   * - 0.785398185 ≈ +pi/4（45°）
   * - 2.3561945   ≈ +3pi/4（135°）
   * 具体 W1/W2 对应哪个轮子/哪个方向，取决于上层接线（main.c 里如何赋值）。
   */
  ...
  /* Outport: '<Root>/YOUT' */
  /* 输出：YOUT 通常作为 Δy（本周期 y 方向位移增量） */
  rtY.YOUT = rtb_MathFunction;

  /* Outport: '<Root>/XOUT' */
  /* 输出：XOUT 通常作为 Δx（本周期 x 方向位移增量） */
  rtY.XOUT = rtb_Sum4;
