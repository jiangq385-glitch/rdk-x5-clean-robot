...
如社区用户名、密码正确，则会在你的家目录下生成`~/.ros/rdk_ai_gateway/auth.bin`文件。
...
2. 本项目已将client打包成节点，便于用户后续撰写自己节点时用于参考。继续在`~/ros2_ws/src/rdk_ai_gateway_ros/rdk_ai_gateway/config/param.yaml`中配置当前prompt，即input_str，和model。运行：`ros2 run rdk_ai_gateway client --ros-args --params-file ~/ros2_ws/src/rdk_ai_gateway_ros/rdk_ai_gateway/config/param.yaml`
