...
If the community username and password are correct, the `auth.bin` file will be generated in your home directory under `~/.ros/rdk_ai_gateway`.
...
```
ros2 run rdk_ai_gateway service --ros-args --params-file ~/ros2_ws/src/rdk_ai_gateway_ros/rdk_ai_gateway/config/param.yaml
```
...
4. This project has packaged the client into a node for reference when users write their own nodes later. Continue to configure the current prompt, i.e. input_str, and model in `~/ros2_ws/src/rdk_ai_gateway_ros/rdk_ai_gateway/config/param.yaml`. Run: `ros2 run rdk_ai_gateway client --ros-args --params-file ~/ros2_ws/src/rdk_ai_gateway_ros/rdk_ai_gateway/config/param.yaml`
